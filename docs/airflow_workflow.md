...
│       ├── tiki_extract.py      # [Extract] Crawl toàn bộ Beauty products → raw JSON
│       ├── tiki_load_iceberg.py # [Load]    Bronze + Silver Iceberg tables
│       └── tiki_gold.py         # [Transform] Gold aggregates → Iceberg + Postgres
│
├── dags/
│   ├── tiki_pipeline_dag.py     # DAG hàng ngày: extract → Bronze/Silver → Gold
